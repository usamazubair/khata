...
/* ── navigation ────────────────────────────────────────────────────────────
   Every module is hand-built, so its navbar is a fixed list defined here.
   Pages set window.khataNav to one of:
     { home: true }
     { module: "transactions", active: "transactions.html" }
     { module: "workout", active: "exercises.html" }
     { admin: true }                                                        */
...
const MODULE_NAV = {
  ...
  workout: {
    ...
    label: "Workout",
    links: [
      { href: "workout.html", label: "Overview" },
...
function renderNav() {
  ...
  const mod = nav.module ? MODULE_NAV[nav.module] : null;

  let left = `<a class="nav-logo" href="index.html">📒 Khata</a>`;
  let links = "";

  if (mod) {
    left = `<a class="nav-back" href="index.html">‹ Modules</a><span class="nav-module">${mod.icon} ${escapeHtml(mod.label)}</span>`;
    links = mod.links
      .map((l) => `<a href="${l.href}" class="${l.href === nav.active ? "active" : ""}">${escapeHtml(l.label)}</a>`)
      .join("");
  } else if (nav.admin) {
  ...
  const adminLink =
    currentUser?.role === "admin" && !nav.admin ? `<a class="nav-user-link" href="users.html">Users</a>` : "";
